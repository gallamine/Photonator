...
rec_pos = [0,0; 0,0; 0,0; 0,0];   % receivers at the same position (on the x/y plane)
...
rec_aperture = [0.0079,0.0079, 0.0508, 0.0508]; % rec_aperture = ones(num_rx,1).*0.0508;  % 0.0508 m = 2 inches
% rec_fov = ones(num_rx,1).*0.314159;     % 0.314159 = 18 deg FOV; ;  % 0.0508 m = 2 inches
rec_fov = [0.01,2.27, 0.0785, 0.1745];     % 0.314159 = 18 deg FOV, 2.27 = 130 deg, 0.0785 = 5 deg
...
total_var_power = zeros(num_rx,1);
total_mean_power = zeros(num_rx,1);
total_mean_angle_sum = zeros(num_rx,1);
total_mean_power_sum = zeros(num_rx,1);
...
parfor simcount = 1:num_sims
    ...
    total_variance_angle = total_variance_angle + (angle_var'.*ph_cnt');
    total_mean_angle_sum = total_mean_angle_sum + angle_mean';
    total_var_power = total_var_power + (power_var'.*ph_cnt');
    total_mean_power_sum = total_mean_power_sum + power_mean';
    run_counter = run_counter+1;

    if (strcmp(saveOutput,'true'))
        parsave(power,ph_cnt,angle_mean,angle_var,power_mean,power_var,num_photons,simcount);
    end

end

total_mean_angle = total_mean_angle_sum / num_sims;
total_mean_power = total_mean_power_sum / num_sims;

total_var_power = total_var_power ./ total_photons;       % SUM(Exp_Variance*Exp_Photons)/Total_Photons
total_variance_angle = total_variance_angle ./ total_photons;

total_power/(num_photons*num_sims)               % Total received power / total transmitted photon packets
sd_power = sqrt(total_var_power).*total_photons/(num_photons*num_sims);     % sd_power = sd_photon*rx_photons/total_photons.
