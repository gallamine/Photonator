...
function [power,ph_cnt,angle_mean,angle_var,distance_mean,distance_var] = mc_rec_r1(a,rec_loc_final,total_rec_dist,rec_pos,rec_aperture,rec_fov)
...
for j = 1:num_rx
   ...
   for i = 1:num_photons
       ...
       distance = sqrt((ph_y-rx_y)^2 + (ph_z-rx_z)^2);
       
       if ((distance <= radius) && (abs(rec_loc_final(i,3)) <= rec_fov(j)/2))   % Photon received
          power(j) = power(j) + exp(-a*total_rec_dist(i));
          ph_cnt(j) = ph_cnt(j) + 1;
          
          angle_mean(j) = ((ph_cnt(j)-1)*angle_mean(j) + rec_loc_final(i,3)) / ph_cnt(j);   % E[theta] = (N*E[theta] + theta) / N+1 
          angle_sqravg = ((ph_cnt(j)-1)*angle_sqravg + rec_loc_final(i,3)^2)/ph_cnt(j);
          angle_var(j) = (ph_cnt(j)/(ph_cnt(j)-1))*(angle_sqravg - angle_mean(j)^2);
